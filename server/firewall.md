...
Standard: `ufw` auf dem Server. Bietet der Anbieter eine externe Cloud-Firewall, dort dieselben vier Ports freigeben.
`install.sh` richtet ufw automatisch ein (Abschnitt 4). Docker veroeffentlicht nur die vier Ports unten, alles andere ist zu.
...
Stattdessen: nur die vier Ports oben in `docker-compose.yml` veroeffentlichen, so wie es jetzt ist.

## Pruefen

```bash
ss -tulpn | grep -E ':(80|443|8189|8889)\b'
# erwartet: 80, 443, 8189 auf 0.0.0.0 (docker-proxy), 8889 nur innerhalb des Compose-Netzes
```
